...
var scrollFrames = function(scrollFramesCSS) {
	...
	var applyScrollFrameAnimations = function(element) {
		...
		//console.log(scrollElements, animationFrameData);
	...
	var applyScrollEventsToContainer = function(container) {
		container.on("touchstart", function() {
			...
			if(scrolling === false) {
				scrolling = true;
				container.on("touchmove", getScrollOffset);
				renderLoop();
	...
	var getElementsScrollFrames = function(animationElements) {
		for(var i = 0; i < animationElements.length; i++) {
			...
			animationElements[i].startFrame = animationElements[i].endFrame - windowHeight;
		}
